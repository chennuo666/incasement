...
namespace incasement.Models
{
    ...
    {
        public static Program SmartChoice(ContainerType ct, CargoNum[] cn,ref int type)
        {
            int cargo_num = getCargoNum(cn);
            double container_v = ct.height * ct.length * ct.width;
            double cargo_v = 0;
            Program result;
            foreach(CargoNum value in cn)
            {
                cargo_v += (value.length * value.width * value.height * value.num);
            }
            cargo_v /= cargo_num;
            double f = Math.Pow(Math.E, cargo_v / container_v) + cargo_num;//评价函数：((货物平均V)/容器V)*货物数量，即等于货物总体积除以容器总体积
            if (f <= 40 && (f - cargo_num) <= 1.64872 && (f - cargo_num) >= 1.00050)//分别为e^(1/2)和e^(1/2000)
            {
                type = 3;
                result = CHNN_3D(ct, cn);
            }
            else if (f > 40 && f <= 100&&(f-cargo_num)<=1.64872&& (f - cargo_num)>=1.00050)
            {
                type = 4;
                result = HGSAA_3D(ct, cn);
            }
            else if(f>100&&f<=1000 && (f - cargo_num) <= 1.64872 && (f - cargo_num) >= 1.00050)
            {
                type = 2;
                result = GA_3D(ct, cn);
            }
        ...
        public static Program HGSAA_3D(ContainerType ct, CargoNum[] cn)
        {
            ...
            double delta = 0.98;
            double finalTemperature = 10;//800度最后0.04度阿尔法700挺好的
            double alpha = 700;
            double point = 64;//5/6对应151度和最后107.5度
            int popnum = 70;
            int cargos_num = getCargoNum(cn);
            double[][] init_pop = getRandomCargoSort(popnum, cargos_num);
